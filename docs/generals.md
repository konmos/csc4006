...
This document presents a brief study of the Byzantine Generals Problem and explores the value of the story of the Byzantine Generals as a software abstraction. Multiple approaches to a software implementation are analysed and a summary of findings, observations, and anything else at all relevant is presented.
...
## User Stories
From this point onwards, the above excerpt of story was treated as a software specification. The first step in ariving at a real software implementation was identifying any potential user stories that would best describe the problem presented in the story.
...
It is important to note here that most (if not all) of these user stories are inferences based on the specification. In actuality, it is unclear, for example, what the motivation of the generals is. Likewise, the Byzantine Generals paper provides contradictory statements about whether the generals wish to share observations, decisions, or a plan of action. The subtle terminology inconsistencies and contradictions introduce a great deal of difficulty in arriving at a purely "objective" software implementation based on the "specification" (story).
...
Following from the previous section, below is a summarised table showing all the pieces of text that in some way "update" the Byzantine Generals story. Excerpts which are clearly a story and nothing more or less, have been labelled "S", while those that belong to the engineering or, for other reasons, do not "qualify" to be called a story are marked NS. Cases in which it is difficult to make the distinction have been labelled by "S/NS".
